...
from tunnel.core.protocol import Message, MessageType, create_tcp_data, create_tcp_close
...
class TCPClientHandler:
    ...
    async def handle_tcp_connect(self, payload: Dict):
        ...
        if not connection_id or not remote_host or not remote_port:
            return
        ...
        try:
            ...
            conn = ClientTCPConnection(
                connection_id=connection_id, reader=reader, writer=writer
            )
    ...
    async def handle_tcp_data(self, payload: Dict):
        ...
        if not connection_id or not data:
            return
    ...
    async def close_connection(self, connection_id: str, reason: Optional[str] = None):
        ...
        if conn:
            ...
            print(
                f"[TCP Client] Closed connection {connection_id}: {reason or 'Unknown'}"
            )
