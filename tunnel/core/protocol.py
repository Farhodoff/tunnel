...
def create_error(
    ...
    payload = ErrorPayload(code=code.value, message=message, details=details)
...
def create_ping() -> Message:
    ...
    return Message.create(
        MessageType.PING, {"timestamp": datetime.utcnow().isoformat()}
    )
...
def create_pong(ping_timestamp: str) -> Message:
    ...
    return Message.create(
        MessageType.PONG,
        {"ping_timestamp": ping_timestamp, "timestamp": datetime.utcnow().isoformat()},
    )


# TCP helper functions
def create_tcp_connect(
    connection_id: str, remote_host: str, remote_port: int
) -> Message:
    """Create TCP connect message"""
    payload = TCPConnectPayload(
        connection_id=connection_id, remote_host=remote_host, remote_port=remote_port
    )
...
def create_tcp_data(connection_id: str, data: str, direction: str = "out") -> Message:
    ...
    payload = TCPDataPayload(
        connection_id=connection_id, data=data, direction=direction
    )
...
def create_tcp_close(connection_id: str, reason: Optional[str] = None) -> Message:
    ...
    payload = TCPClosePayload(connection_id=connection_id, reason=reason)
